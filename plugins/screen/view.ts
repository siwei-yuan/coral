...
import { ScreenRuntime } from "./runtime.ts"

export class ScreenView implements ViewExtension {
  readonly plugin = "screen"
  readonly title = "Screen"
  readonly runtime: ScreenRuntime

  constructor(runtime: ScreenRuntime) {
    this.runtime = runtime
  }

  async render(): Promise<string> {
    const activity = await this.runtime.current()
    ...
    return `<style>
      .screen-head{display:flex;justify-content:space-between;gap:18px;align-items:flex-start;padding-bottom:18px;border-bottom:1px solid #dce7f2}.screen-head h2{margin:3px 0}.screen-meta{text-align:right;color:#61758d}
      .screen-grid{display:grid;gap:18px;margin-top:20px}.screen-capture{display:grid;grid-template-columns:minmax(260px,1.35fr) minmax(220px,1fr);gap:18px;padding:14px;border:1px solid #dce7f2;border-radius:18px;background:#f8fbff}
      .screen-capture img{width:100%;border-radius:12px;border:1px solid #dce7f2;background:white}.screen-capture h3{margin:4px 0 9px}.screen-capture pre{margin:0;white-space:pre-wrap;font:13px/1.55 ui-monospace,SFMono-Regular,Menlo,monospace;color:#334d68}
