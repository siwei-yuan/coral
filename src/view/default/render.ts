...
function page(title: string, content: string, extensions: ViewExtensionLink[], active?: string): string {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)} · Coral</title><style>${styles}</style></head><body class="${active ? "extension-page" : "overview-page"}"><nav><a class="brand" href="/" aria-label="Coral"><img src="/_view/coral-wordmark.png" alt="Coral"></a><div class="nav-items"><a class="nav-link${active ? "" : " active"}" href="/"${active ? "" : ` aria-current="page"`}>Overview</a>${links}</div></nav><main>${content}</main><script>${viewScript}</script></body></html>`
}
