...
const initializeTopology = sector => {
  ...
  const viewport = sector.querySelector('[data-topology-viewport]'), plane = sector.querySelector('[data-topology-plane]')
  ...
  let token = 0, playing = false, scale = 1, x = 0, y = 0, drag = null
  const draw = () => { plane.style.transform='translate('+x+'px,'+y+'px) scale('+scale+')' }
  const fit = () => {
    const width=Number(plane.dataset.width),height=Number(plane.dataset.height)
    scale=Math.min(1,(viewport.clientWidth-32)/width,(viewport.clientHeight-32)/height)
    x=(viewport.clientWidth-width*scale)/2;y=(viewport.clientHeight-height*scale)/2;draw()
  }
  const zoom = (factor,cx=viewport.clientWidth/2,cy=viewport.clientHeight/2) => {
    const next=Math.min(1.8,Math.max(.3,scale*factor)),worldX=(cx-x)/scale,worldY=(cy-y)/scale
    x=cx-worldX*next;y=cy-worldY*next;scale=next;draw()
  }
  viewport.addEventListener('pointerdown',event=>{drag={px:event.clientX,py:event.clientY,x,y};viewport.setPointerCapture(event.pointerId);viewport.classList.add('dragging')})
  ...
  const endDrag = event => {drag=null;viewport.classList.remove('dragging');if(viewport.hasPointerCapture(event.pointerId))viewport.releasePointerCapture(event.pointerId)}
...
@media(max-width:520px){.brand img{width:64px}.nav-link{padding:6px 10px}.hero h1{font-size:2.25rem}.surface{border-radius:22px}}
@media(prefers-reduced-motion:reduce){*{scroll-behavior:auto!important;animation-duration:.01ms!important;animation-iteration-count:1!important;transition-duration:.01ms!important}.evolution-plane,.topology-plane{will-change:auto}.topology-edge{transition:none}}
